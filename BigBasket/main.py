...
def create_excel_file(categories):
    workbook = xlsxwriter.Workbook('E:\VegEase\BigBasket\Bigbasket_products.xlsx')
...
def main_function(url, category_name, existing_df):
    ...
    conn = sqlite3.connect('E:\VegEase\BigBasket\Bigbasket_products_database.db')
    c = conn.cursor()
    ...
    inserted_products = load_inserted_products('E:\VegEase\BigBasket\Bigbasket_inserted_products.txt')
    ...
    for card in bigbasket_product_cards:
        ...
        quantity_elem = card.find("span", class_="Label-sc-15v1nk5-0 PackChanger___StyledLabel-sc-newjpv-1 gJxZPQ cWbtUx")
...
if __name__ == "__main__":
    ...
    if not os.path.isfile('E:\VegEase\BigBasket\Bigbasket_products.xlsx'):
